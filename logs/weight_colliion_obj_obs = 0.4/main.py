...
class Main:
    def __init__(self):
        ...
        self.histories = np.zeros([self.updates, 5]) # reward, std of reward, length, std of length
    ...
    def run_training_loop(self):
        ...
        for update in range(self.updates):
            ...
            self.histories[update,:] = [
                ...
                np.min(rewards), np.max(rewards), np.mean(lengths)
            ]

            print(f"Iteration:{update:<4} reward:{self.histories[update, 0]:<.3}±{self.histories[update,1]:<.3} min:{self.histories[update, 2]:<.3} max:{self.histories[update, 3]:<.3} ")
